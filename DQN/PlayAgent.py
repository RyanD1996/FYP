...
# ==========================================================================================
import pong # My PyGame Pong Game
import DQN.Agent as Agent # My DQN Based Agent
import numpy as np 
import random 
#
import pickle, os
...
def PlayGame():
	...
	while (GameTime < TOTAL_GAMETIME):    
		...
		BestAction = 0
		BestAction = TheAgent.return_best_action(GameState)

		#  Now Apply the Recommended Action into the Game 	
		[ReturnScore,NewScreenImage]= TheGame.get_next_frame(BestAction, user_action=None)
		if(ReturnScore == -1):
			user_score += 1
		...
		if GameTime % 25 == 0:
			...
			print("Game Time: ", GameTime,"  Game Score: ", "{0:.2f}".format(TheGame.tally), "   EPSILON: ", "{0:.4f}".format(TheAgent.epsilon))
			GameHistory.append((GameTime,TheGame.tally,TheAgent.epsilon))
